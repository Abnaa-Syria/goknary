...
const AdminAnnouncementPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.error || 'Failed to update broadcast');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-8 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        ...
        <p className="text-gray-500 mt-1 text-sm tracking-wide">Manage the scrolling announcement bar at the top of your marketplace</p>
      </div>
          ...
          <section className="bg-white rounded-[40px] border border-gray-100 shadow-2xl shadow-gray-200/50 overflow-hidden">
            <div className="p-8 border-b border-gray-50 bg-gray-50/50 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-black text-gray-900 uppercase tracking-tighter">Broadcast Content</h2>
                  <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest">Single record overwrite logic active</p>
                </div>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-10 space-y-8">
              <div className="space-y-6">
                <div className="space-y-2">
                  <label className="text-xs font-black uppercase tracking-widest text-gray-400 ms-1">Announcement Message</label>
                  <textarea
                    value={data.message}
                    ...
                    placeholder="Example: Free Shipping on all orders over 500 EGP! Use code KANARY20."
                    required
                  />
                  <div className="flex justify-between items-center px-2">
                    <p className="text-[10px] text-gray-400 italic">This text will loop infinitely at the top of every page.</p>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-primary-600 rounded-[40px] p-8 text-white shadow-2xl shadow-primary-200 relative overflow-hidden group">
             ...
             <p className="text-sm font-medium leading-relaxed opacity-90">
                Keep messages concise (under 100 characters) for the best reading experience. High-impact emojis can significantly increase engagement with your promotional offers!
             </p>
          </div>
          ...
          <div className="bg-white rounded-[40px] border border-gray-100 p-8 shadow-xl shadow-gray-100/50">
             ...
             <div className="bg-black rounded-2xl h-10 flex items-center overflow-hidden relative">
                <div className="flex animate-marquee whitespace-nowrap">
                   <span className="text-[10px] font-bold text-white uppercase px-4">{data.message || 'No message configured...'}</span>
                   <span className="text-[10px] font-bold text-white uppercase px-4">{data.message || 'No message configured...'}</span>
                </div>
             </div>
             <p className="text-[10px] text-gray-400 mt-4 text-center italic">Visualization of the top ticker bar</p>
          </div>
        </div>
      </div>
    </div>
