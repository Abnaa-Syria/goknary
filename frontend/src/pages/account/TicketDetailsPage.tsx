...
const TicketDetailsPage: React.FC = () => {
  ...
  const getStatusText = (status: TicketDetails['status']) => {
    switch (status) {
      case 'OPEN':
  ...
  const getPriorityText = (priority: TicketDetails['priority']) => {
    switch (priority) {
      case 'LOW':
  ...
  const fetchTicketDetails = async () => {
    try {
      ...
      toast.error(t('account.ticketsPage.fetchFailed'));
  ...
  const handleSendReply = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(t('account.ticketsPage.messageSent'));
      setReplyText('');
      fetchTicketDetails();
    } catch (error: any) {
      console.error('Failed to send reply:', error);
      toast.error(error.response?.data?.error || t('account.ticketsPage.sendReplyFailed'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleCloseTicket = async () => {
    if (!window.confirm(t('account.ticketsPage.closeConfirm'))) return;

    try {
      await api.patch(`/tickets/${id}/status`, { status: 'CLOSED' });
      toast.success(t('account.ticketsPage.closeSuccess'));
      fetchTicketDetails();
    } catch (error: any) {
      console.error('Failed to close ticket:', error);
      toast.error(error.response?.data?.error || t('account.ticketsPage.closeFailed'));
    }
  };
    ...
    <div className="space-y-6 flex flex-col h-[75vh] text-start">
      <div className="flex items-center justify-between flex-shrink-0 border-b border-gray-100 pb-4">
        ...
        >
          ...
          {t('account.ticketsPage.backToTickets')}
        </Link>
        ...
        {ticket.status !== 'CLOSED' && ticket.status !== 'RESOLVED' && (
          ...
          >
            <XCircle size={14} />
            {t('account.ticketsPage.closeTicket')}
          </button>
        )}
      </div>

      <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm flex-shrink-0">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
          <h3 className="text-lg font-black text-gray-900 leading-tight">{ticket.subject}</h3>
          <div className="flex gap-2">
            ...
            <span className="px-2 py-0.5 bg-gray-100 text-gray-500 rounded text-[10px] font-black uppercase tracking-wider">
              {t('account.ticketsPage.priorityLabel', { priority: getPriorityText(ticket.priority) })}
            </span>
          </div>
        </div>
        <p className="text-sm text-gray-600 leading-relaxed bg-gray-50 p-4 rounded-xl border border-gray-100/50">
          {ticket.message}
        </p>
        <p className="text-[10px] text-gray-400 mt-2 font-medium">
          {t('account.ticketsPage.openedOn')}{' '}
      ...
      <div className="flex-1 min-h-0 bg-gray-50/50 rounded-2xl border border-gray-100 p-4 overflow-y-auto custom-scrollbar flex flex-col space-y-4">
        {ticket.messages.length === 0 ? (
          <div className="my-auto text-center p-6">
            ...
            <p className="text-xs text-gray-400 font-medium">{t('account.ticketsPage.noRepliesYet')}</p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="space-y-1">
                  <div className={`text-[10px] font-bold text-gray-400 ${isAgent ? '' : 'text-end'}`}>
                    {msg.sender.name || t('account.ticketsPage.supportAgent')}{' '}
                    {isAgent && (
                      <span className="bg-secondary-50 text-secondary-900 text-[8px] font-black rounded-full px-1.5 py-0.5 border border-secondary-100 ms-1">
                        {t('account.ticketsPage.agentBadge')}
                      </span>
                    )}
                  </div>
                  <div
      ...
      <div className="flex-shrink-0">
        {ticket.status === 'CLOSED' || ticket.status === 'RESOLVED' ? (
          <div className="flex gap-2 items-center justify-center p-4 bg-gray-100 rounded-xl text-xs font-semibold text-gray-500">
            <AlertCircle size={14} />
            {t('account.ticketsPage.ticketClosedNotice')}
          </div>
        ) : (
          <form onSubmit={handleSendReply} className="flex gap-2">
            <input
              ...
              placeholder={t('account.ticketsPage.replyPlaceholder')}
              required
